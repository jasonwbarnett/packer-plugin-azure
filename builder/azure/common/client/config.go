...
// `client_id` and `subscription_id` are specified with one and only one of the
// follow: `client_secret`, `client_jwt`, `client_cert_path` -- Packer will use
// the specified Azure Active Directory (AAD) Service Principal (SP).  If only
// `use_interactive_auth` is specified, Packer will try to interactively log on
// the current user (tokens will be cached).  If none of these options are
// specified, Packer will attempt to use the Managed Identity and subscription
// of the VM that Packer is running on.  This will only work if Packer is
// running on an Azure VM with either a System Assigned Managed Identity or User
